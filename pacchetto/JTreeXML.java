...
public class JTreeXML extends JTree {
	...
	private DefaultMutableTreeNode radiceJTree = new DefaultMutableTreeNode(new Nodo("","",null,Nodo.TIPONODO_FILE));
	...
	public void caricaXML(String nomeFile)
	{
		try
		{
			...
			radiceJTree.setUserObject(new Nodo(nomeFile,"",null,Nodo.TIPONODO_FILE));
	...
	public void salvaXML(String nomeFile)
	{
		...
		org.jdom.Element radice = new org.jdom.Element("Radice");
	...
	public boolean aggiungiAttributo(DefaultMutableTreeNode nodo,String nome,String testo)
	{
		...
		if (ok)
		{
			ele.getAttributi().put(nome,nome + "=" + testo);
		}
	...
	private void scandisciJTree2XML(org.jdom.Element nodoXML,DefaultMutableTreeNode nodoJTree,boolean primo)
	{
		if (nodoJTree!=null)
		{
			...
			{
				String[] temp = attr[t].toString().split("=");
	...
	private void scandisciXML2JTree(Node nodoXML,DefaultMutableTreeNode nodoJTree)
	{
		...
		if (nodoXML.getNodeType()==Node.ELEMENT_NODE)
		{
			...
			{
				elenco.put(nodoXML.getAttributes().item(t).getNodeName(),nodoXML.getAttributes().item(t).getNodeName() + "=" + nodoXML.getAttributes().item(t).getNodeValue());
			}
